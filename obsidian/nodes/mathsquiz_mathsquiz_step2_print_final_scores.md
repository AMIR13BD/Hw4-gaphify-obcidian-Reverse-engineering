# print_final_scores()

Back: [[../index|index]] · Hot list: [[../hot|hot]]

## Identity

- **Node id:** `mathsquiz_mathsquiz_step2_print_final_scores`
- **Source file:** `mathsquiz/mathsquiz-step2.py`
- **Community:** 2
- **Type:** `code`

## Degree (from metrics)

- In-degree: 1
- Out-degree: 0
- Total degree: 1

## Incoming Relations

- `contains` from [[nodes/mathsquiz_mathsquiz_step2|mathsquiz-step2.py]] (EXTRACTED)

## Outgoing Relations

- _(none)_

## Investigation Notes

- Graph evidence ranks this node; it is **not** confirmed as a bug or design flaw.
- Validate claims by opening the source file and related tests manually.
- Cross-check confidence in [[../reports/graph_summary|graph_summary]].
