"""Tests for obsidian CLI command."""
...
def test_cli_obsidian_rejects_invalid_phase() -> None:
    """Parser only accepts before/after phases."""
    parser = build_parser()
    with pytest.raises(SystemExit):
        parser.parse_args(["obsidian", "--phase", "middle"])


def test_cli_obsidian_with_fixture(tmp_path, monkeypatch) -> None:
    """Obsidian subcommand writes vault files from fixtures."""
    ...
    (phase_dir / "GRAPH_REPORT.md").write_text("# Graph Report", encoding="utf-8")
    (metrics_dir / "metrics_before.json").write_text(
        fixtures.joinpath("metrics_sample.json").read_text(encoding="utf-8"),
        encoding="utf-8",
    )

    exit_code = main(["obsidian", "--phase", "before"])
    payload_path = project_root / "obsidian" / "index.md"

    assert exit_code == 0
    assert payload_path.is_file()
    vault_index = payload_path.read_text(encoding="utf-8")
    assert "Graph Summary" in vault_index


def test_cli_obsidian_missing_metrics_fails(tmp_path, monkeypatch) -> None:
    """Obsidian returns non-zero when metrics file is missing."""
    _setup_project(tmp_path, monkeypatch)
    assert main(["obsidian", "--phase", "before"]) == 1
